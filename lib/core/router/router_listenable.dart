...
class RouterListenable extends _$RouterListenable implements Listenable {
  ...
  Future<void> build() async {
    ...
    _isAuth = await ref.watch(
      authenticationProvider.selectAsync(
  ...
  String? redirect(BuildContext context, GoRouterState state) {
    ...
    if (isSplash) {
      return _isAuth ? UserRoute.path : LoginRoute.path;
    }

    if (_isAuth) {
      switch (state.uri.toString()) {
        case LoginRoute.path:
        case RegisterRoute.path:
        case ForgetPasswordRoute.path:
          WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
            navigatorKey.currentContext?.go(UserRoute.path);
          });
      ...
      if (url != LoginRoute.path ||
          url != RegisterRoute.path ||
          url != ForgetPasswordRoute.path) {
        WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
          navigatorKey.currentContext?.go(LoginRoute.path);
        });
